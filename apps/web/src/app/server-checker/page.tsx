...
const ServerCheckerContent = () => {
	...
	const fetchData = useCallback(async () => {
		...
		try {
			const res = await fetch(`/api/server-checker?userid=${session?.user?.id}`, {
				headers: {
					...
					<div className='bg-background/60 w-full p-5 md:p-8 rounded-md'>
						{loading || status === 'loading' ? (
							...
							<div className='flex flex-wrap flex-col w-full gap-8'>
								<div className='flex flex-wrap gap-12 bg-background/50 p-5 rounded-md shadow-lg'>
									...
									<div className='flex flex-col justify-center'>
										<p>
											<strong className='font-semibold'>Random hash:</strong> {randomHash}
										</p>
										<p>
											<strong className='font-semibold'>User ID:</strong> {session.user.id}
										</p>
										<p>
								...
								{guilds.length > 0 ? (
									<>
										<div>
											...
											<p>
												The servers listed below are flagged as bad servers. To proceed with
												your appeal (if you have one open), you must leave these servers.
											</p>
											<p>
												After leaving the servers, click on "Do another check" to verify if
												you are still in any blacklisted servers.
											</p>
										</div>
										<Table>
											<TableHeader>
											...
											<TableBody>
												{guilds.map((guild: Guild) => (
													<TableRow key={guild.id}>
														...
														<TableCell className='w-[30%]'>{guild.id}</TableCell>
														<TableCell className='w-[70%]'>{guild.name}</TableCell>
													</TableRow>
												))}
											</TableBody>
										</Table>
									...
									<>
										<div>
											...
											<p>
												<strong>You are all good!</strong> To proceed with your appeal (if
												you have one open), click the "Download as png" button below and
												send it in your appeal ticket.
											</p>
										</div>
									</>
								)}
							...
							<>
								...
								<p className='mb-5'>
									Log in with your Discord account to see if you are part of any servers
									that have been flagged as blacklisted.
								</p>
