...
import { getCache, lruInfinity, setCache } from "../../utils/cache";
...
async function dmUser(
	...
	const [imports, allImports] = await Promise.all([
		importsPromise,
		allImportsPromise,
	]);
...
async function actionUser(
	...
	const settings = await db.getGuild(
		{ id: guildId },
		{ logChannel: true, punishments: true },
	);
	...
	if (settings?.punishments?.roleId)
		await member.roles.remove(
			settings?.punishments?.roleId,
			"Removing role to be actioned",
		);
...
async function processMessage(client: Client, msg: amqp.ConsumeMessage | null) {
	if (msg) {
		...
		await setCache(
			"consumerCount",
			((await getCache("consumerCount", lruInfinity)) || 0) + 1,
			lruInfinity,
		);
		...
		messageQueue.push(async () => {
			try {
				...
				if (punishment === "BAN" || punishment === "KICK")
					await actionUser(client, id, guildId, punishment);
				channel!.ack(msg);
				...
				const consumerCount =
					(await getCache("consumerCount", lruInfinity)) || 0;
				await setCache(
					"consumerCount",
					consumerCount > 0 ? consumerCount - 1 : consumerCount,
					lruInfinity,
				);

				// Wait 2s between each dmUser
				await new Promise((resolve) => setTimeout(resolve, 2000));

				// Every 10 DMs, wait 10s seconds
				if (messageCount % 10 === 0) {
					logger.info({
						message: `Processed ${messageCount} users, waiting 10 seconds`,
					});
...
async function logQueueStatus() {
	if (channel) {
		logger.info({
			message: `Total users to be actioned left in queue: ${await totalQueue()}`,
		});
	}
}
...
async function startReceiver(client: Client) {
	...
	const queue = "actionUser";
	channel.consume(queue, (msg) => processMessage(client, msg), {
		noAck: false,
	});
