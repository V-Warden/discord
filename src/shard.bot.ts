...
export class Sharding {
  static start(): void {
    ...
    manager.on('shardCreate', (shard) => {
      logger.debug(`Successfully launched shard ${shard.id} of ${(manager.totalShards as number) - 1}`);
      shard.on('death', (process) => {
        logger.error(
          'Shard ' + shard.id + ' closed unexpectedly! PID: ' + process.pid + '; Exit code: ' + process.exitCode + '.'
        );
