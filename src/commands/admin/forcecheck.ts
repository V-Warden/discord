    ...
    await client.guilds.cache.reduce(async (a, guild) => {
      ...
      await guild.members
        ...
        .then(async (member) => {
          ...
          client.actioning.doAction(user, settings.logChannel, settings.punishments, member, false, false, false);

          client.logger.debug(`forceCheck ${id}: Finished actioning ${guild.name}`);
        })
        .catch(() => client.logger.debug(`forceCheck ${id}: Skipping ${guild.name} not in guild`));
    }, Promise.resolve());

    const end = Date.now();
    client.logger.info(
      `forceCheck ${id}: Finished actioning on ${client.guilds.cache.size} guilds, took ${(end - begin) / 1000}s`
    );
