      ...
      await guild?.members
        ...
        .then(async (members) => {
          ...
          if (toAction.length > 0) {
            ...
            client.logger.info(`${this.constructor.name}: ${guild.name} - Actioning ${toAction.length} users`);
            ...
            await toAction.reduce(async (a, member) => {
              ...
              client.actioning.doAction(member, db.logChannel, db.punishments, realMember, false, true, rescan);
              await new Promise((resolve) => setTimeout(resolve, 100));
            }, Promise.resolve());
            client.logger.debug(`${this.constructor.name}: ${guild.name} - Finished actioning`);
          } else {
            client.logger.debug(`${this.constructor.name}: ${guild.name} - Skipping no one to action`);
            return;
          }
        })
        .catch(() => {
