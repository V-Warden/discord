    ...
    interaction.guild.members
      ...
      .then(async () => {
        ...
        if (!settings.punishments.enabled) {
          sendEmbed({
            interaction,
            embed: {
              description: '`🔴` Actioning is disabled on this server, you can enable it using `/config`',
              color: Colours.RED,
            },
          });
          return false;
      ...
      .catch((e) => {
        ...
        sendEmbed({
          ...
          embed: {
            description:
              'An unexpected error has occured. If re-trying this command does not work, please open a support ticket in the Official Warden Discord',
            color: Colours.RED,
          },
        }).catch((e) => console.log(e));
      });
